...
class CommandGetPersonDetails extends Cubit<StateCommandGetPersons> {
  CommandGetPersonDetails({int id}) : super(const StateCommandGetPersons.loadInProgress(0.0)) {
    _run(id: id);
  }
    ...
    try {
      ...
      if (response?.statusCode == 200) {
        if (response.data is Map) {
          ...
          emit(StateCommandGetPersons.data(_person));
        } else {
...
abstract class StateCommandGetPersons with _$StateCommandGetPersons {
  ...
  const factory StateCommandGetPersons.data(Person person) = _Data;
